...
from src.visualization import (
    ...
    plot_agent_route, plot_all_routes_optimised, plot_greedy_vs_optimised,
    _build_zones_coords
)
...
def main():
    ...
    zones_coords = _build_zones_coords(warehouse)
    ...
    greedy_total_dist = sum(
        ...
        for loc in o.get_unique_pick_points()
    )
    ...
    # 6. route individuelle par agent
    for route_info in route_results:
        agent_id = route_info['agent_id']
        plot_agent_route(
            route_info,
            wh_dict,
            zones_coords=zones_coords,
            save_path=f"results/route_{agent_id}.png"
        )

    # 7. toutes les routes optimisees superposees
    plot_all_routes_optimised(
        route_results, wh_dict,
        zones_coords=zones_coords,
        save_path="results/routes_all_optimised.png"
    )

    # 8. comparaison glouton vs optimise
    plot_greedy_vs_optimised(
        greedy_result, route_results, wh_dict,
        zones_coords=zones_coords,
        greedy_total_dist=greedy_total_dist,
        save_path="results/routes_greedy_vs_optimised.png"
    )
    ...
    print("  - route_R1.png, route_R2.png, ...  (tournee individuelle)")
    print("  - routes_all_optimised.png          (toutes les tournees)")
    print("  - routes_greedy_vs_optimised.png    (comparaison)")
